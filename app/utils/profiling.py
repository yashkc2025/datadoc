...
def profile_dataset(df: pd.DataFrame) -> dict:
    ...
    for col in df.columns:
        ...
        col_profile["missing_pct"] = round((series.isnull().sum() / len(series)) * 100, 2) if len(series) > 0 else 0.0

        if pd.api.types.is_numeric_dtype(series):
            col_profile["mean"] = round(series.mean(), 2) if series.notnull().any() else None
            col_profile["median"] = round(series.median(), 2) if series.notnull().any() else None
            col_profile["std"] = round(series.std(), 2) if series.notnull().any() else None
            col_profile["min"] = round(series.min(), 2) if series.notnull().any() else None
            col_profile["max"] = round(series.max(), 2) if series.notnull().any() else None
            col_profile["skewness"] = round(skew(series.dropna()), 2) if series.notnull().any() else None
            col_profile["kurtosis"] = round(kurtosis(series.dropna()), 2) if series.notnull().any() else None
            ...
            col_profile["most_frequent"] = series.mode().iloc[0] if not series.mode().empty else None
            col_profile["top_5_values"] = series.value_counts().nlargest(5).to_dict()
        elif pd.api.types.is_datetime64_any_dtype(series):
            col_profile["first_date"] = series.min().strftime('%Y-%m-%d') if series.notnull().any() else None
            col_profile["last_date"] = series.max().strftime('%Y-%m-%d') if series.notnull().any() else None
